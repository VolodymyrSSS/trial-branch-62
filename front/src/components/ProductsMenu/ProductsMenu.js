...
class ProductsMenu extends Component {
  ...
  render() {
	...
	const productMenuDesc = productsMenu.map((obj, index) => {
		if(obj.lvl === 1) {
			...
			return(
				<li key={obj._id} className="nav-item dropdown brand-list_item" id={obj._id}>
					...
					{ productsMenu.map((obj, index) => {
						if(obj.lvl === 2 && obj.parent === firstLvlRender){
							const secondLvlRender = obj._id;
							return(
								<div key={obj.name} className="item-menu__wrapper">
									<Link className="dropdown-item" to='/' id={obj.id}>{obj.name}</Link>
									<div className="dropdown-divider"></div>
									{productsMenu.map((obj, index) => {
										
										if(obj.lvl === 3 && obj.parent === secondLvlRender){
											return(
												<Link key={obj.categoryFilter.model} className="dropdown-item_down" to='/' id={obj._id}>{obj.name}</Link>
											)
										}
									})}
								</div>
	...
	const productMenuTabl = productsMenu.map((obj, index) => {
		if(obj.lvl === 1) {
			const firstLvlRender = obj._id;
			return(
				<li key={obj._id} className="nav-item dropdown brand-list_item" id={obj._id}>
