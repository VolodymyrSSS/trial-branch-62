    ...
    handleInputChange = event => {
      ...
      handleFormSubmit = event => {
        ...
        if (validation.isValid) {	
			this.props.registrationCustomerAction(this.inputEmail.value,
		...
		return (
			<React.Fragment>
				...
				<Header/>
					<Row>
						<div
						className="jumbotron col-12 p-0 bg-white min-vh-100 text-center m-0 d-flex 
...
const mapDispatchToProps = function (dispatch) {
    return {
		registrationCustomerAction: (email, login, firstName, lastName, secondPass, history) => dispatch(registrationCustomer(email, login, firstName, lastName, secondPass, history)),
    }
};
