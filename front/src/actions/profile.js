...
import {EDIT_PROFILE_PASS_REQUEST, EDIT_PROFILE_REQUEST, CUSTOMER_REQUEST, GET_CUSTOMER} from './action';
...
export function editProfile(email, login, firstName, lastName) {
    ...
    return async dispatch => {
        ...
        await axios.put("/profile/editprof",{
            data:{email, login, firstName, lastName}
        })
        .then(res => { 
            if(res.status === 200){
                dispatch({
